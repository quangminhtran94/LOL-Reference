$(document).on('pageinit', "#champion-detail",function () {
	...
	$.ajax({url: BASE_URL + STATIC_DATA_CATEGORY + NA_REGION + VERSION + CHAMPION_CATEGORY + "/"+id+"?"+LOCATION+"="+language+ "&champData=image,stats,info,spells,passive&"+API_KEY, success: function(result){
		...
		$.each(result.spells, function(index, value){
			...
			$('#champion-spells').append('<li><img src="' + BASE_SPELL_IMAGE_URL +value.image.full+'"><p>' + '<b>' + value.name + ' ' +skillKeyboard[index%4]+ '</b><br>' + cost + SpellToString(value, value.tooltip) +'</p></li>').listview("refresh");
		});
	...
	if(!isItemsLoaded){
		...
		$.ajax({url: BASE_URL + STATIC_DATA_CATEGORY + NA_REGION + VERSION + CHAMPION_CATEGORY + "/"+id+"?"+LOCATION+"="+language+ "&champData=recommended&"+API_KEY, success: function(result){
			$.each(result.recommended, function(index, value){
				if((value.map =="SR"|| value.map=="1") && value.mode == "CLASSIC"){
					displayRecommendedItems(value.blocks);
					isItemsLoaded = true;
				}
			});
			if(!isItemsLoaded){
				$.each(result.recommended, function(index, value){
					if( value.map=="10" && value.mode == "CLASSIC"){
						displayRecommendedItems(value.blocks);
						isItemsLoaded = true;
					}
				});
...
$(document).on("click", ".item-image", function(){
	var id = $(this).attr('id');
	$("#ItemPopup p").empty();
	$.ajax({url: BASE_URL + STATIC_DATA_CATEGORY + NA_REGION + VERSION + ITEM_CATEGORY + "/"+id+"?"+LOCATION+"="+language+ "&itemData=gold&"+API_KEY, success: function(result){
		$("#ItemPopup p").append('<b>'+result.name+'</b><br>');
		$("#ItemPopup p").append(result.description);
	}});
	$("#ItemPopup").popup("open");
})

});  

function displayRecommendedItems(blocks){
	$.each(blocks, function(index, value){
		var type = value.type.toLowerCase();
		$("#recommended-items").append("<li id='"+type+"'><p><b id='"+type+"-title'></b></p></li>");
		changeLang(language);
		$.each(value.items, function(index, value){
			...
			$.ajax({url: BASE_URL + STATIC_DATA_CATEGORY + NA_REGION + VERSION + ITEM_CATEGORY + "/"+value.id+"?"+LOCATION+"="+language+ "&itemData=image&"+API_KEY , success: function(result){
				for(i = 0; i < numberOfItem; i++){
					var selector='#'+type.toLowerCase();
					$(selector).append('<img class="item-image" id="'+result.id+'" src="' + BASE_ITEM_IMAGE_URL + result.image.full +'">');
				}	
			}});
		});
	});
